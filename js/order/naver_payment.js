...
fetch('http://kdt-sw-5-team01.elicecoding.com/api/orderslist', {
    ...
    .then((orderData) => {
        fetch(
            `http://kdt-sw-5-team01.elicecoding.com/api/ordersitemlist/${
                orderData[orderData.length - 1]._id
            }`,
            {
                method: 'GET',
                headers: {
            ...
            .then((itemData) => {
                if (itemData.length == 1) {
                    ...
                    itemName.innerHTML = `${itemData[0].productName} 외 ${
                        itemData.length - 1
                    }개`;
                }
                itemPrice.innerHTML = `${itemData[0].orderId.totalPrice.toLocaleString(
                    'ko-KR',
                )}원`;
                totalPrice1.innerHTML =
                    itemData[0].orderId.totalPrice.toLocaleString('ko-KR');
                totalPrice2.innerText = `${itemData[0].orderId.totalPrice.toLocaleString(
                    'ko-KR',
                )}원`;
                ...
                mileage.innerText = (
                    itemData[0].orderId.totalPrice * 0.01
                ...
                useAllPointBtn.addEventListener('click', () => {
                    ...
                    totalPayment.innerText = (
                        itemData[0].orderId.totalPrice - 50000
                    ).toLocaleString('ko-KR');
                    totalPriceFin.innerText = (
                        itemData[0].orderId.totalPrice - 50000
                    ).toLocaleString('ko-KR');
                    mileage.innerText = (
                        (itemData[0].orderId.totalPrice - 50000) *
                    ...
                    usePoint.addEventListener('focus', () => {
                        if (usePoint.value !== '') {
                            let numberString = usePoint.value.replace(
                                /[^\d.-]/g,
                                '',
                            );
                ...
                usePoint.addEventListener('blur', () => {
                    ...
                    let sum = Number(
                        itemData[0].orderId.totalPrice - usePoint.value,
                    );
                    givenPoint.innerText = (
                        givePoint - usePoint.value
                    ).toLocaleString('ko-KR');
                    usePoint.value = Number(usePoint.value).toLocaleString(
                        'ko-KR',
                    );
                    usePartPoint.innerText = usePoint.value;
...
cardTab.addEventListener('click', () => {
    ...
    document.getElementsByClassName('_payMethodForm')[0].style.display =
        'block';
});
